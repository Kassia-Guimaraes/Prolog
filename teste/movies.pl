/* DATABASE
    ...
    movie(M, G, Y) <- movie M of genre G came out in year Y
    ...
    Genres: Adventure, Comedy, Crime and mystery, Drama, Fantasy, Historical, Horror, Romance, Science fiction, Other
*/
...
movie(american_beauty, drama, 1999).
...
movie(barton_fink, adventure, 1991).
...
movie(the_big_lebowski, adventure, 1998).
...
movie(blade_runner, adventure, 1997).
...
movie(blood_simple, adventure, 1984).
...
movie(the_cotton_club, drama, 1984).
...
movie(crimewave, adventure, 1985).
...
movie(down_from_the_mountain, adventure, 2000).
...
movie(frankenweenie, adventure, 1984).
...
movie(ghost_busters, adventure, 1984).
...
movie(girl_with_a_pearl_earring, adventure, 2003).
...
movie(the_godfather, adventure, 1972).
...
movie(the_godfather_part_ii, adventure, 1974).
...
movie(the_godfather_part_iii, adventure, 1990).
...
movie(groundhog_day, adventure, 1993).
...
movie(hail_caesar, adventure, 2006).
director(hail_caesar, ethan_coen).
director(hail_caesar, joel_coen).

movie(hearts_of_darkness_a_filmmaker_s_apocalypse, adventure, 1991).
...
movie(the_hudsucker_proxy, adventure, 1994).
...
movie(inside_monkey_zetterland, adventure, 1992).
...
movie(intolerable_cruelty, adventure, 2003).
...
movie(the_ladykillers, adventure, 2004).
...
movie(lick_the_star, adventure, 1998).
...
movie(lost_in_translation, adventure, 2003).
...
movie(the_man_who_wasn_t_there, adventure, 2001).
...
movie(marie_antoinette, adventure, 2006).
...
movie(miller_s_crossing, adventure, 1990).
...
movie(mission_impossible, adventure, 1996).
...
movie(no_country_for_old_men, adventure, 2007).
director(no_country_for_old_men, joel_coen).

movie(o_brother_where_art_thou, adventure, 2000).
...
movie(the_outsiders, adventure, 1983).
...
movie(paris_je_t_aime, adventure, 2006).
...
movie(peggy_sue_got_married, adventure, 1986).
...
movie(raising_arizona, adventure, 1987).
...
movie(rumble_fish, adventure, 1983).
...
movie(spies_like_us, adventure, 1985).
...
movie(star_wars_episode_i__the_phantom_menace, adventure, 1999).
...
movie(torrance_rises, adventure, 1999).
...
movie(the_usual_suspects, adventure, 1995).
...
movie(the_virgin_suicides, adventure, 1999).
...
movie(an_american_rhapsody, adventure, 2001).
...
movie(the_black_dahlia, adventure, 2006).
...
movie(eight_legged_freaks, adventure, 2002).
...
movie(ghost_world, adventure, 2000).
...
movie(a_good_woman, adventure, 2004).
...
movie(if_lucy_fell, adventure, 1996).
...
movie(home_alone_3, adventure, 1997).
...
movie(the_horse_whisperer, adventure, 1998).
...
movie(in_good_company, adventure, 2004).
...
movie(just_cause, adventure, 1995).
...
movie(the_island, adventure, 2005).
...
movie(a_love_song_for_bobby_long, adventure, 2004).
...
movie(manny__lo, adventure, 1996).
...
movie(match_point, adventure, 2005).
...
movie(my_brother_the_pig, adventure, 1999).
...
movie(the_perfect_score, adventure, 2004).
...
movie(the_spongebob_squarepants_movie, adventure, 2004).
...
movie(untitled_woody_allen_fall_project_2006, adventure, 2006).
...
movie(a_view_from_the_bridge, adventure, 2006).
actress(a_view_from_the_bridge, scarlett_johansson, catherine).
actor(a_view_from_the_bridge, anthony_lapaglia, eddie_carbone).

movie(harry_potter_and_the_goblet_of_fire, adventure, 2005).
director(harry_potter_and_the_goblet_of_fire ,mike_newell).
actress(harry_potter_and_the_goblet_of_fire, emma_watson, hermione_granger).
actor(harry_potter_and_the_goblet_of_fire, daniel_radcliffe, harry_potter).
actor(harry_potter_and_the_goblet_of_fire, rupert_grint, ron_weasley).

same_genre(Movie_1, Movie_2, Genre):-
    movie(Movie_1, Genre, _), movie(Movie_2, Genre, _), Movie_1 \= Movie_2.
