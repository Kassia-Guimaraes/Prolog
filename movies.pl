...
released_after(M,Y) :-
    movie(M, B), B > Y.
    /*,write("The movie "), write(M), write(" was released after the year "), write(Y), tab(8).*/

released_before(M, Y) :-
    movie(M, B), B < Y.
	/*,write("The movie "), write(M), write(" was released before the year "), write(Y), tab(8).*/

same_year(A, B) :-
    movie(A, Y), movie(B, Y), A \= B.
	/*,write("The movies "), write(A),write(" and "), write(B), write(" are released in "), write(Y), tab(8).*/

co_star(A, B) :-
    (actor(X, A,_), actress(X, B,_));
    (actress(X, A,_), actress(X, B,_));
    (actor(X, A,_), actor(X, B,_));
    (actress(X, A,_), actor(X, B,_)), 
    A \= B.
    /*,write(A), write(" and "), write(B), write(" are in the movie "), write(X), tab(8).*/